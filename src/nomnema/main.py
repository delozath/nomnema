...
class BibtexParserDriver:
    def __init__(self, cfg: DictConfig):
        ...
        """
        bib_db = cfg.bib_database
        if bib_db is None:
            self.bibfile = (
                Path('.').absolute() / "temp.bib"
            )
        else:
            self.bibfile = Path(bib_db)
        
        if not self.bibfile.exists():
            self.bibfile.write_text("", encoding="utf-8")    
...
class PDFFileManager:
    def __init__(self, cfg: DictConfig):
        ...
        """
        self.pth_org = self._folder_exists(cfg.path_origin)
        self.pth_dest = self._folder_exists(cfg.path_destination)
    
    def _folder_exists(self, pfname: str) -> Path:
        """
        Check if a directory/folder exists at the given path.
        Parameters
        ----------
        pfname : str
            Path to the directory/folder to check.
        
        Returns
        -------
        Path
            The folder path if it exists, otherwise raises an error.
        ...
        if path.is_dir():
            ...
            raise NotADirectoryError(f"`{pfname}` is not a folder")
...
def orchestrator(cfg: DictConfig) -> None:
    ...
    pfname = Path(cfg.path_origin, cfg.fname).as_posix()
    result = md.convert(pfname)
    breakpoint()
    bib_db = BibtexParserDriver(cfg)
    bib_db.append_entry(bib_entry)
    bib_db.save()
    print(bib_entry)
    
    pdf_manager = PDFFileManager(cfg)
